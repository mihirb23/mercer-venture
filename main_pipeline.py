...
from processing_the_documents import DocumentProcessor
...
def full_pipeline_with_rag_setup(pdf_path: str, bilstm_model=None, bigru_model=None):
    ...
    raw_text, tables = doc_processor.process_document(pdf_path)
    logging.info("Document extraction complete.")
    ...
    cleaned_text_output_path = os.path.join(output_dir, f"{document_name}_cleaned.txt")
    processed_json_output_path = os.path.join(output_dir, f"{document_name}_processed_data.json")
    try:
        with open(cleaned_text_output_path, "w", encoding="utf-8") as f:
            f.write(cleaned_text)
        with open(processed_json_output_path, "w", encoding="utf-8") as f:
            json.dump(processed_data, f, indent=2, ensure_ascii=False)
        logging.info(f"Saved cleaned and processed data for {pdf_path}")
    except Exception as e:
        logging.error(f"Failed to save cleaned or processed data: {e}")

    text_chunker = TextChunker(chunk_size=500, chunk_overlap=100)
    chunks = text_chunker.create_chunks(cleaned_text)
    logging.info(f"Text chunking complete. Total chunks: {len(chunks)}")

    # Chunk-level classification
    if bilstm_model is not None:
        for chunk in chunks:
            chunk_pred = bilstm_model.infer(chunk["content"])
            chunk["bilstm_classification"] = chunk_pred
    if bigru_model is not None:
        for chunk in chunks:
            chunk_pred = bigru_model.infer(chunk["content"])
            chunk["bigru_classification"] = chunk_pred

    embedding_generator = EmbeddingGenerator(model_name="all-MiniLM-L6-v2")
    chunk_contents = [chunk["content"] for chunk in chunks]
    chunk_embeddings = embedding_generator.generate_embeddings(chunk_contents)
    logging.info("Embeddings generated for all chunks.")

    redis_url = os.getenv("REDIS_URL", "redis://localhost:6379")
    redis_index_name = "insurance_docs_index"
    embedding_dimension = 384
    redis_db_manager = RedisVectorDBManager(redis_url, redis_index_name, embedding_dimension)
    ...
    logging.info(f"Chunks and embeddings for {pdf_path} stored in Redis.")
...
if __name__ == "__main__":
    ...
    try:
        ...
        for pdf_file in pdf_files:
            ...
            print("#" * 70 + "\n")
            try:
                full_pipeline_with_rag_setup(
                    file_path, bilstm_model=bilstm_model, bigru_model=bigru_model
                )
            except Exception as e:
                logging.error(f"An error occurred while processing {pdf_file}: {e}", exc_info=True)
    except Exception as e:
